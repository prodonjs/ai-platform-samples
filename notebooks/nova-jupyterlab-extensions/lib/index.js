...
const gcpService = new gcp_1.GcpService(new gapi_1.GapiService());
/**
...
const buttonExtension = {
    ...
    activate: (app) => {
        ...
        app.docRegistry.addWidgetExtension('Notebook', new ButtonExtension(gcpService));
    },
    autoStart: true,
};
const jobsExtension = {
    ...
    activate: (app, restorer) => {
        ...
        const sidePanel = new jobs_1.JobsWidget();
    ...
    createNew(panel, context) {
        const button = new apputils_1.ToolbarButton({
            ...
            onClick: () => __awaiter(this, void 0, void 0, function* () {
                console.log('Submit Notebook clicked');
                const content = panel.model.toString();
                // console.log(await this.gcpService.getEnabledServices());
                this.gcpService.uploadNotebook(content, 'notebooks/client/upload/test');
            }),
...
//         var instance_types =
//             instance_types_per_gpu_type_per_count[gpu_type][gpu_cout];
...
//     var trainingTargets = [
//       'gce',
//       // "local" - local trainig is not yet supported and should be reanabled
//       as
...
//     return {
//       'gpu_type':
//           (<HTMLInputElement>this.node.querySelector('#gpuTypeInput')).value,
//       'gpu_count':
//           +(<HTMLInputElement>this.node.querySelector('#gpuCountInput')).value,
//       'instance_type':
//           (<HTMLInputElement>this.node.querySelector('#instanceTypeInput'))
//               .value,
//       'local':
//       (<HTMLInputElement>this.node.querySelector('#trainingTypeInput'))
//                    .value == 'local',
//       'parameter':
//           (<HTMLInputElement>this.node.querySelector('#parameterInput')).value
//     };
//   }
// }
// Export the extensions
exports.default = [buttonExtension, jobsExtension];
